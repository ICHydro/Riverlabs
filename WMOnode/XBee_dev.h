...
public:
	...
	PayloadRequest(uint8_t apiId, uint8_t frameId, EEPROMClass, uint16_t payloadLength);
	/**
	 * Returns the payload of the packet, if not null
	 */
	uint8_t* getPayload();
	// WB: overloaded function:
	uint8_t getPayload(uint16_t index);
	/**
...
private:
	...
	bool _ReadFromEEPROM = false;  // WB addition
};
...
public:
	...
	*/
	...
	IPTxRequest(uint32_t addr32, uint16_t port, EEPROMClass, uint16_t payloadLength);
	/**
	...
	*/
	...
	IPTxRequest(uint32_t addr32, uint16_t port, uint16_t sourcePort, uint8_t protocol, uint8_t option, EEPROMClass, uint16_t payloadLength, uint8_t frameId);
	/**
