...
**Q2: Is battery voltage >3.3 V?**

- Measure with multimeter at battery terminals
- **No** (<3.3 V) → **SOLUTION: Replace/charge battery**
- **Yes** (≥3.3 V) → Go to Q3
...
- **Never changes (e.g., stuck at 5000 mm)** → Go to Q3
- **Changes slightly** → Go to Q6

**Q3: What is the constant value?**

- **5000 mm or max range** → **SOLUTION: Sensor timeout, no target detected**
...
- **Offset by constant amount** (e.g., always 500 mm too high) → Go to Q7
...
- **Example:** Sensor 3000 mm above water, reads 1500 mm → water depth = 3000 - 1500 = 1500 mm
...
- Check: Voltage >3.5 V (telemetry requires more power than SD-only)
- **<3.5 V** → **SOLUTION: Charge/replace battery, retest**
- **≥3.5 V** → Go to Q6
...
- **"Disk not initialized" or similar**
    - **SOLUTION: Format card** (FAT32, allocation unit 32 KB)
...
- Check properties
- **>32 GB** → **SOLUTION: Card too large, use ≤32 GB**
- **≤32 GB** → Go to Q6
