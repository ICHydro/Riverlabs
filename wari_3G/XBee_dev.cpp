...
PayloadRequest::PayloadRequest(uint8_t apiId, uint8_t frameId, uint8_t *payload, uint16_t payloadLength) : XBeeRequest(apiId, frameId) {
	...
	_ReadFromEEPROM = false;
}

PayloadRequest::PayloadRequest(uint8_t apiId, uint8_t frameId, EEPROMClass EEPROM, uint16_t payloadLength) : XBeeRequest(apiId, frameId) {
	_payloadLength = payloadLength;
	_ReadFromEEPROM = true;
}

// WB: Deprecated function.
...
uint8_t ZBTxRequest::getFrameData(uint16_t pos) {
	if (pos == 0) {
		...
		return getPayload(pos - ZB_TX_API_LENGTH);  // WB change
	}
}
...
uint8_t ZBExplicitTxRequest::getFrameData(uint16_t pos) {
	if (pos < 10) {
		...
		return getPayload(pos - ZB_EXPLICIT_TX_API_LENGTH);  // WB change
	}
}
...
uint8_t Tx16Request::getFrameData(uint16_t pos) {
	...
	if (pos == 0) {
		...
		return getPayload(pos - TX_16_API_LENGTH);  // WB change
	}
}
...
uint8_t Tx64Request::getFrameData(uint16_t pos) {
	...
	if (pos == 0) {
		...
		return getPayload(pos - TX_64_API_LENGTH);   // WB change
	}
}
...
void XBee::send(XBeeRequest &request) {
	...
	// WB: small change to avoid repeated call to getFrameData()
	for (int i = 0; i < request.getFrameDataLength(); i++) {
		uint8_t b = request.getFrameData(i);
		sendByte(b, true);
		checksum+= b;
	}
	// end WB
...
IPTxRequest::IPTxRequest(uint32_t addr32, uint16_t port, uint16_t sourcePort, uint8_t protocol, uint8_t option, EEPROMClass EEPROM, uint16_t payloadLength, uint8_t frameId) : PayloadRequest(TX_IP4_REQUEST, frameId, EEPROM, payloadLength) {
	_hostIP = addr32;
	_port = port;
	_sourcePort = sourcePort;
	_protocol = protocol;
...
IPTxRequest::IPTxRequest(uint32_t addr32, uint16_t port, EEPROMClass EEPROM, uint16_t payloadLength) : PayloadRequest(TX_IP4_REQUEST, DEFAULT_FRAME_ID, EEPROM, payloadLength) {
	_hostIP = addr32;
	_port = port;
	_sourcePort = 0;
	_protocol = XC_TCP_PROTOCOL;
...
uint8_t IPTxRequest::getFrameData(uint16_t pos) {
	if (pos == 0) {
		...
		return getPayload(pos - TX_IP4_API_LENGTH);  // WB change
	}
}
