...
void RioLogger::write2EEPROM(byte *EEPromPage, uint8_t length) {
  ...
  i2c_eeprom_write_page(EEPROM_ADDR, (eePageAddress + EEPromHeaderSize) * EEPromPageSize,
                        EEPromPage, length);

  delay(50); // to get rid of some issues with writing to EEPROM (first entry sometimes wrong)

  // mark page as 1 in EEPromSDMask (offset 1 page), EEProm3GMask (offset 33 pages), and EEPromMask
  // (offset 65 pages)

  for (uint8_t i = 0; i < 3; i++) {
    byte EEPROMbyte = i2c_eeprom_read_byte(
        EEPROM_ADDR, offset[i] + (uint16_t)eePageAddress / 8); // read the relevant byte
    bitWrite(EEPROMbyte, eePageAddress % 8, 1);                // toggle relevant bit
    i2c_eeprom_write_byte(EEPROM_ADDR, offset[i] + (uint16_t)eePageAddress / 8,
                          EEPROMbyte); // write relevant byte
  }
...
// Note that data can be maximum of about 30 bytes, because the Wire library has a buffer of 32
// bytes and this includes the two address bytes
