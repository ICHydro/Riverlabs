...
void printSendAndWaitError(Stream &stream) { stream.print(F("sendAndWait() error: ")); }
...
void tcpSend(uint32_t ip, uint16_t port, uint8_t protocol, uint8_t *payload,
...
void zbModemStatusCb(ModemStatusResponse &mx, uintptr_t) {
  ...
  if (modemStatus == 2) {
    // Serial.println(F("Registered with the cellular network"));
    MyXBeeStatus.isRegistered = true;
  } else if (modemStatus == 3) { // NOLINT: branches are identical
    ...
    MyXBeeStatus.isRegistered = false;
  } else if (modemStatus == 0x0E) { // NOLINT: branches are identical
  ...
  if (atr.getCommand()[0] == 'L' || atr.getCommand()[1] == 'A') {
    if (atr.isOk()) {
      if (atr.getValueLength() == 4) {
        ...
        IP = (((uint32_t)atr.getValue()[0]) << 24) + (((uint32_t)atr.getValue()[1]) << 16) +
             (((uint32_t)atr.getValue()[2]) << 8) + atr.getValue()[3];
        // Set flags
...
void sendXbeeMessage(uint8_t *buffer, uint16_t bufferSize, char *host, uint8_t hostlength) {
  ...
  if (!MyXBeeStatus.xbcErrorOccurred && !MyXBeeStatus.ipResponseReceived) {
    ...
    if (!MyXBeeStatus.hostIPResolved) {
      ...
      // A request is made to lookup an IP address and response handled in callback function
      // zbAIResponseCb
      ...
      // There is nothing to do here as the processing is handled in callback function
      // zbIPResponseCb
    }
...
void sendXbeeMessage(uint16_t bufferSize, char *host, uint8_t hostlength) {
  ...
  if (!MyXBeeStatus.xbcErrorOccurred && !MyXBeeStatus.ipResponseReceived) {
    ...
    } else if (!MyXBeeStatus.ipRequestSent) { // NOLINT
      // Send the request
      // The response is handled in callback function zbTcpSendResponseCb
      Serial.print(F("Sending TCP request to "));
      Serial.print(IP, HEX);
...
bool setclock_ntp() {
  ...
  uint16_t Port =
      123; // 0x50 = 80; 0x1BB = 443, 0x1633 = 5683 (COAP), 0x75B = 1883 (MQTT), 1337 = NTP
