...
void InterruptServiceRoutine() { interruptFlag = true; }
...
int16_t median(int16_t samples[], int m) // calculate the median
{
  ...
  if (bitRead(m, 0) == 1) { // If the last bit of a number is 1, it's odd. This is equivalent to
                            // "TRUE". Also use if m%2!=0.
    return sorted[m / 2];   // If the number of data points is odd, return middle number.
  } else {
    return (sorted[(m / 2) - 1] + sorted[m / 2]) /
           2; // If the number of data points is even, return avg of the middle two numbers.
  }
}
...
void formatDateTime(const RtcDateTime &dt) {
  snprintf_P(datestring, countof(datestring), PSTR("%04u/%02u/%02u %02u:%02u:%02u"), dt.Year(),
...
uint8_t CreateEepromSendBuffer(uint16_t start, byte *mask) {
  ...
  // as avr boards do not support the C++ bitset, one potential alternative is to use a uint64_t as
  // bit array and do byteshifting.
  ...
  if (count == 0) {
    return count;
  } // if there aren't any data to send, we can stop here.
  ...
  for (i = 0; i < MAXFIT; i++) {
    ...
    if ((mask[index] >> j) & 0x1) {
      ...
      i2c_eeprom_read_buffer(EEPROM_ADDR, (i + start + EEPromHeaderSize) * EEPromPageSize,
                             EEPromPage, readsize);
      if (first) {
        ...
        for (k = 0; k < 10; k++) {
          EEPROM.write(
              starttime++,
              timestamp[k]); // use write instead of put to avoid writing the null character
        }
      }
      ...
      for (k = 0; k < 5; k++) {
        EEPROM.write(starth++,
                     value[k]); // use write instead of put to avoid writing the null character
      }
...
uint16_t CreateSendBuffer(uint16_t start, byte *mask2, uint8_t *buffer, uint16_t bufferSize) {
  ...
  uint16_t readsize = (EEPromPageSize > 30) ? 30 : EEPromPageSize;
  i2c_eeprom_read_buffer(EEPROM_ADDR, (start + EEPromHeaderSize) * EEPromPageSize, EEPromPage,
                         readsize);
  dataout = (int16_t *)EEPromPage;
...
void Reset3GBuffer(uint16_t start, byte *oldmask) {
  ...
  uint8_t count = 0;
  uint16_t i, j, index;
  byte newmask[2 + MAXFIT / 8]; // Note that this may be different form the old mask if more data
                                // were written in the meanwhile!

  // read the mask again, in case any bits have changed (new records created) after oldmask was
  // created
