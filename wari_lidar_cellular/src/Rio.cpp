...
uint8_t CreateSendBuffer(uint16_t start, byte *mask, uint8_t *buffer) {
    ...
    if ((mask[0] >> j) & 0x1) {
        ...
        bufferSize += 10;
        memcpy((char*) (buffer + bufferSize), string2, 19);
        bufferSize += 18;
        sprintf((char*) (buffer + bufferSize), "%5d", dataout[4]);
        bufferSize += 5;
        memcpy((char*) (buffer + bufferSize), string3, 7);
        bufferSize += 5;
        sprintf((char*) (buffer + bufferSize), "%5d", dataout[2]);
        bufferSize += 5;
        memcpy((char*) (buffer + bufferSize), string4, 7);
        bufferSize += 5;
        sprintf((char*) (buffer + bufferSize), "%5d", dataout[3]);
        bufferSize += 5;
        memcpy((char*) (buffer + bufferSize), string5, 3);
        bufferSize += 2;
        return(1);
    } else {
        return(0);
