...
#define EEPROM_ADDR                                                                                \
  0x51 // EEPROM I2C address: 0x57 for AT24c32 on clock; 0x51 or 81 for chip on PCB (Node_3G, SD
       // boards)
#define EEPromPageSize                                                                             \
  32 // max 32 bytes for the AT24c32, up to 128 bytes for M24512. Can be smaller (e.g., 8, 16)
#ifdef M24512
...
#define EEPromMaskSize                                                                             \
  32 // mask to keep track of what pages have written data on them (used for cycling)
#define EEPromHeaderSize                                                                           \
  (1 + EEPromMaskSize + EEPromSDMaskSize +                                                         \
   EEProm3GMaskSize) // gives 3999 useable pages for M24512 with 16 byte pages (13 days at 5 min
                     // intervals)
#define MAXFIT                                                                                     \
  50 // maximum number of records that fits in the EEPROM; will depend on format and number of
     // variables to be transmitted.
     // TODO: can be calculated automatically
#define OFFSET3GMASK                                                                               \
  (1 + EEPromSDMaskSize) * EEPromPageSize // starting position of 3GMASK in EEPROM
...
#include <avr/power.h>
#include <avr/wdt.h>
// #include <RH_RF95.h>         // only needed if a LoRa radio is present on the board, to put it in
// sleep mode.
