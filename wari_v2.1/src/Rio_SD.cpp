...
void turnOnSDcard() {
  // Riverlabs comment: it seems more stable to set pins 10, 11, and 13 first before powering on the
  // SD card. Otherwise some boards restart when the SD card is switched on. Maybe a power surge
  // through one of the pins? Still need to look into the following comment from Mallon: "some cards
  // will fail on power-up unless SS is pulled up  ( &  D0/MISO as well? )"
  DDRB = DDRB | (1 << DDB5) | (1 << DDB3) |
         (1 << DDB2); // set SCLK(D13), MOSI(D11) & SS(D10) as OUTPUT
  // Note: | is an OR operation so  the other pins stay as they were.                (MISO stays as
  // INPUT)
  PORTB = PORTB &
          ~(1 << DDB5); // disable pin 13 SCLK pull-up – leave pull-up in place on the other 3 lines
  delay(10);
...
void turnOffSDcard() {
  ...
  PORTB |= ((1 << DDB5) | (1 << DDB4) | (1 << DDB3) |
            (1 << DDB2)); // set ALL SPI pins HIGH (~30k pullup)
...
uint8_t dumpEEPROM2() {
  ...
  if (!SD.begin(SD_CS_PIN, SPI_FULL_SPEED)) {
    ...
    if (fileopen) {
      ...
      fileopen = 0; // set to 0 whatever the outcome of close() because the SD card will be powered
                    // off anyway.
    }
  }
#ifdef DEBUG
...
uint8_t writeEEPROMline(uint16_t n) {
  ...
  i2c_eeprom_read_buffer(EEPROM_ADDR, (n + EEPromHeaderSize) * EEPromPageSize, EEPromPage2,
                         readsize);
